...
public class NPC_target_script : MonoBehaviour {
    ...
    public NPC_cannon_script cannon;
	...
	void Start () {
        ...
        cannon = gameObject.GetComponent<NPC_cannon_script>();
        ship = transform.parent.GetComponent<Rigidbody2D>();
    ...
    private void move_to(Vector3 dest)
    {
        ...
        if (delta_a > 0)
        {
            this.transform.parent.Rotate(new Vector3(0, 0, -Time.deltaTime * angular_speed));
        }
        if (delta_a < 0)
        {
            this.transform.parent.Rotate(new Vector3(0, 0, +Time.deltaTime * angular_speed));
